    ...
    <section className="py-16 sm:py-28 px-6 sm:px-8 bg-white">
      <div className="max-w-[85.5rem] mx-auto">
        <Reveal className="text-center mb-6 sm:mb-8">
          <h2 className="font-serif text-3xl sm:text-4xl md:text-5xl lg:text-6xl text-[#1a2332] mb-3">How We Begin</h2>
        ...
        <div className="flex flex-col items-center gap-10 lg:hidden">
          ...
          {allPhasesOrdered.map((phase, index) => (
            <Reveal key={phase.title} delay={index * 100} className="text-center">
              <span className="block font-serif text-base text-[#1a2332] mb-1">{phase.numeral}</span>
        ...
        <div className="hidden lg:grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] grid-rows-2 items-start gap-x-6 gap-y-12">
          {leftPhases.map((phase, index) => (
            <Reveal
              key={phase.title}
              delay={index * 120}
              className={`text-center pr-10 col-start-1 ${index === 0 ? "row-start-1" : "row-start-2"}`}
            >
          ...
          <Reveal delay={100} className="w-[34rem] self-center col-start-2 row-start-1 row-span-2">
            <div className="relative aspect-square overflow-hidden mb-3">
              <Image
                src="/images/4226.webp"
                alt="Abstract colorful painting with heavy texture"
          ...
          {rightPhases.map((phase, index) => (
            <Reveal
              key={phase.title}
              delay={200 + index * 120}
              className={`text-center pl-10 col-start-3 ${index === 0 ? "row-start-1" : "row-start-2"}`}
            >
              <span className="block font-serif text-2xl text-[#1a2332] mb-1">{phase.numeral}</span>
              <h3 className="font-serif text-3xl text-[#1a2332] mb-3">{phase.title}</h3>
              <p className="text-sm sm:text-base text-slate-600 leading-relaxed">{phase.description}</p>
